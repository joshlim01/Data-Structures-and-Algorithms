...
void AVLTree<K, V>::remove(Node*& subtree, const K& key)
{
    ...
    if (key < subtree->key) {
        ...
        if (subtree->left == NULL && subtree->right == NULL) {
            ...
            // Swap keys/values (temporaily violates the search tree property,
            // but that will be restored by the removal below.)
            swap(subtree, pred);

            // Recursively remove the key from the left subtree, rebalancing after that's complete.
            // This wastes some effort refinding the key.
            // If that proves critical in our application, we may want to revisit and optimize this code.
            //
            // Note: Without something that causes rebalancing from the predecessor's position (and not just from subtree),
            // we can end up with an unbalanced tree.
            remove(subtree->left, key);
