...
class SQtree {
 ...
 private:
  ...
  std::pair<int,int> bestSplit(stats & s, pair<int,int> & ul , int w, int h);
  bool areSame(double a, double b);
  void helpRender(Node * node, PNG & im);
  int countNode(Node * &node);
  Node * copyNode(Node * node);
  void clearNode(Node *& node);
  bool validSplit(pair<int, int> ul, int x, int y, int w, int h);
  double maxDouble(double a, double b, double c, double d);
