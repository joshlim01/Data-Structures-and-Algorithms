...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if(distance <= 200) {
      double brightness = 1 - (0.005 * distance);

      double adjusted_red = brightness * pixel->r;
      double adjusted_green = brightness * pixel->g;
      double adjusted_blue = brightness * pixel->b;
...
PNG ubcify(PNG image) {
  ...
  for(unsigned int x = 0; x < image.width(); x++) {
    for(unsigned int y = 0; y < image.height(); y++) {
      //get a handle to the current pixel
      RGBAPixel* px = image.getPixel(x, y);
      //and save the original color
      RGBAPixel origpx(px->r, px->g, px->b, px->a);
      double dist_to_yellow = colordist(origpx, ubcyellow);
      double dist_to_blue = colordist(origpx, ubcblue);

      if(dist_to_yellow <= dist_to_blue) {
        //set current pixel to UBC yellow
        px->r = ubcyellow.r;
        px->g = ubcyellow.g;
        px->b = ubcyellow.b;
      }
        ...
        //set current pixel to UBC blue
        px->r = ubcblue.r;
        px->g = ubcblue.g;
        px->b = ubcblue.b;
      }
      //adjust the brightness based on original pixel's brightness
      double origbrightness = ((double) origpx.r + (double) origpx.g + (double) origpx.b) / (255.0*3);
      px->r *= origbrightness;
      px->g *= origbrightness;
      px->b *= origbrightness;
    }
